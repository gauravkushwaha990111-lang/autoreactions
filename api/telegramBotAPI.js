...
export default class TelegramBotAPI {
    ...
    async callApi(action, body) {
        try {
            const response = await fetch(this.apiUrl + action, {
                ...
                signal: AbortSignal.timeout(40000) // 40 second timeout for long polling
            });
            ...
            if (!response.ok) {
                ...
                if (action === 'setMessageReaction') {
                    ...
                    console.error(`To Chat ID: ${body.chat_id}, From Chat ID: ${body.from_chat_id}`);
                } else if (action === 'answerCallbackQuery') {
                    console.error(`Callback Query ID: ${body.callback_query_id}`);
                } else if (action === 'getUpdates') {
                    // Polling timeout logs ko ignore karein
                } else {
     ...
     * Photos, Videos aur Text ko exactly copy karke bhejne ke liye
     */
     ...
     * Button click ka loading animation rokne ke liye
     */
     ...
     * Bot commands set karne ke liye
     */
     ...
     * Bot commands delete karne ke liye
     */
     ...
     * Localhost polling ke liye updates fetch karna
     */
