...
async function applyReaction(botApi, chatId, message_id, activeReactions, defaultReactions, AdminId) {
    let attempts = 0;
    // Har message par max 5 baar try karein agar reactions invalid ho rahe hain
    let maxAttempts = Math.min(activeReactions.length, 5); 
    
    // Copy activeReactions to avoid modifying the original array if passed by reference
    let currentMessageReactions = [...activeReactions];

    while (attempts < maxAttempts && currentMessageReactions.length > 0) {
        const emoji = getRandomPositiveReaction(currentMessageReactions);
        if (!emoji) break;
        ...
        try {
            ...
            if (error.message && error.message.includes('REACTION_INVALID')) {
                // Sirf is chat/message ke liye is emoji ko ignore karein
                currentMessageReactions = currentMessageReactions.filter(e => e !== emoji);
                attempts++;
            } else {
                // Koi aur error (e.g. Message Deleted, Bot Kicked) toh loop band kar do
                break; 
            }
        }
    }
